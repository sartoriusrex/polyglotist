    ...
    try {
      ...
      if (settings.length > 0) {
        const newSettings = await db.query(
          ...
          [readingSpeed,themePreference, practiceMode, notificationMethod, languagePreference, languagesLearning, username]
        );
        ...
        const newSources = await Promise.all(
          ...
          sources.map( async srcName => {
            try {
              ...
              console.log(err, ' Line 145 users handler, converting src names to ids');
            }
          })
        ).then( sourceIds => {
          ...
          return Promise.all(
            sourceIds.map( async srcId => {
              ...
              try {
                ...
                console.log(err, ' Error inserting into users_sources table line 161 users handler');
              }
            })
          )
        }).then( ids => {
          ...
          return Promise.all(
            ids.map( async id => {
              ...
              try {
                ...
                console.log(err, 'Error line 178, mapping new source ids to their names');
              }
            })
          )
        }).catch( err => {
